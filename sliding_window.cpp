...
int main(int argc, char* argv[])
{
    ...
    if (argc != 3)
    {
        fprintf(stderr, "usage: sliding_window hostname username");
        exit(1);
    }
	...
	*server_info;	   // head of linked list of results from getaddrinfo()
    ...
    unsigned char setup_ack[MAXBUFLEN];

    memset(setup_message, 0, MAXBUFLEN);
    memset(setup_ack, 0, MAXBUFLEN);
	...
	int client_sockfd = socket(PF_INET, SOCK_DGRAM, 0); // create a new UDP socket to send to (the other client)
	if (client_sockfd == -1)
	{
		perror("receiver: socket");
		exit(1);
    ...
    while((setup_ack_received && setup_message_received) != 1)
    {
        ...
        if ((buffer[4] == 0x00) && (buffer[5] == 0x00))
        {
            ...
            setup_ack[0] = 0x00; //seqnum1
            setup_ack[1] = 0x00; //seqnum2
            setup_ack[2] = 0x00; //seqnum3
            setup_ack[3] = 0x00; //seqnum4
            setup_ack[4] = 0x00; //control
            setup_ack[5] = 0x01; //ack
            setup_ack[6] = 0x00; //len1
            setup_ack[7] = 0x00; //len2

            send(argv[1], setup_ack);
        }
    }
